...
propósito for a obtenção de aromáticos, segue-se à hidrogenação seletiva o segundo estágio
...
?? cuidar a formatação de listas, escolher um formato e serguir em todo
documento. Se haverá ponto e vírgula entre itens e o ponto final no final ??
...
expressada de duas maneiras: (i) retenção total ($\epsilon^L$), definida
...
definida como retenção dinâmica \cite{Ranade2011}. Alguns autores utilizam como referência a saturação
...
drenado significará diretamente $\epsilon_{Ld}$. Outra técnica consiste em
...
determinação de $\beta_L$. \citeonline{Larachi1991} utiliza a distribuição
do tempo de residência para chegar a $\beta_L$.
?? cuidar a diferença entre 'citeonline' e 'cite' ??
...
?? não citar equações antes de aparecerem no texto, veja alteração abaixo ??
...
?? A própria relação entre difusividade e velocidade superficial não seria
importante aqui??
...
diesel, por exemplo, o \ce{H2S} produto descola-se parcialmente para a fase
...
\cite{Ancheyta2011}.
?? Não gostei deste parágrafo, existem categorizações melhores: modelos
empírivos \emph{vs.} baseados em primeiros princípios (bal. de massa, energia,
etc.). Acho que vale a pena procurar melhor isto em outras fontes. ??
...
catalisador pode ser de paládio ou níquel suportado em alumina. Normalmente, o reator possui
ao menos dois leitos. Devido a exotermicidade das reações, há uma injeção de
\emph{quench} líquido entre os leitos, composta de produto hidrogenado
resfriado, que serve para controlar a temperatura do segundo leito e evitar o coqueamento sobre
...
Como já mencionado, o grau de hidrogenação dependerá da
utilização final da corrente hidrogenada\cite{Cheng1986}.
...
adicionado um termo de adsorção à cinética de reação das olefinas, uma vez que a reação destas é
...
fizeram. ?? cuirdar a ordem cronológica para explicar, arrumar acima ??
...
?? Este último trabalho parece que não havia sido citado ainda e aqui dá a
entender que sim. Também detalhar melhor o que seria 'não agrupando'.
Talvez listar as substâncias consideradas aqui para ilustar ??
...
adiabático, utilizando a abordagem $\phi-\phi$, onde uma equação
de estado é utilizada para o cálculo de equilíbrio de fases.
Isso foi feito
porque um dos objetivos do trabalho foi comparar diferentes equações de
estado (e seus parâmetros) quanto a solubilidade de hidrogênio em gasolina de pirólise. A
...
\citeonline{Rojas2014a} ?? duas vezes o mesmo??, que é uma extensão do estudo
feito por \citeonline{Zhou2006}.
...
O trabalho de \citeonline{Rojas2014a} desta ?? destaca-se ?? dos demais autores
citados por considerar o regime de escoamento do reator como sendo do tipo borbulhamento,
ainda que ambas as fases estejam em escoamento descendente. Assim, algumas
equações hidrodinâmicas consideradas são diferentes das utilizadas pelos demais
trabalhos.
