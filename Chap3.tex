...
\citeonline{Rojas2014a} foi o de realizar comparações entre os dois estudos.
...
célula $z$, no tempo $t$: 
\begin{equation}
\dfrac{dM_{i,z}}{dt} = F_z \theta_{i,z} - F_{z+1} \theta_{i,z+1} +
...
$r_{j,z}$ é taxa da reação $j$ na célula $z$, $F$ é a vazão molar total,
$\theta$ é a fração molar total do composto $i$ e $W$ é a massa total de
...
\nomenclature[G]{$\theta$}{Fração molar total de um composto}
...
\begin{equation}
E_{z} = M_{z}h_{z+1} - P_{z+1}V_{z} + 
\dfrac{W}{N_{Disc}}C_{p,p}(T_{z}-T^{ref}_2)
\label{eq:holdupenergiadinamica}
\end{equation}
onde $C_{p,p}$ é o $C_p$ do catalisador, \SI{880}{J/(kg K)} e $T^{ref}_2$ é a
...
\nomenclature{$C_p$}{Capacidade calorífica a pressão constante
\nomunit{J/(kg.K)}}
...
equação de van't Hoff, na $T^{ref}_1$ = \SI{417}{K}, como segue:
\begin{equation}
k^{*}_{j,z} = \dfrac{k^{'}_{j,z}} {\rho^{L}_{z+1}} \exp
\left[{\dfrac{-Ea_j}{R} \left (\dfrac{1}{T_{z+1}} -
\dfrac{1}{T^{ref}_1} \right )}\right]
...
\nomenclature{$T^{ref}_1$}{Temperatura de referência, $417 K$}
...
\begin{equation}
\epsilon_BSh^L_{i,z} = \num{0,8}(Re^L_z)^{\num{0,5}}(Sc^L_{i,z})^{1/3} \qquad
(Re<200)
\label{eq:Sh1}
\end{equation}
\begin{equation}
\epsilon_BSh^L_{i,z} = \num{0,53}(Re^L_z)^{\num{0,58}}(Sc^L_{i,z})^{1/3} \qquad
(Re>200)
\label{eq:Sh2}
\end{equation}
sendo que
...
\left(\dfrac{\dfrac{u^{G}}{u^{L}} + 1}{\num{0,49}\dfrac{u^{G}}{u^{L}} +
1}\right) \left[\dfrac{150}{36}\left(\dfrac{6(1-\epsilon_B)}{d_P}+\dfrac{4}{D_R}
\right)^2\mu^{L}u^{L} + \dfrac{\num{1,75}}{6} \left(\dfrac{6(1-\epsilon_B)}{d_P}
