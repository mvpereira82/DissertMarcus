...
\nomenclature {$\rho_{Al_2O_3}$}{Massa específica do óxido de alumínio \nomunit{kg/m^3}}
...
Antes de determinar a porosidade dos leitos catalíticos, é preciso avaliar a massa específica das partículas (\rho_{p}). Este parâmetro, calculado pela \autoref{eq:rhoparticula}, é estimado em $1244 kg/m^3$. 
...
\nomenclature {$\rho_{p}$}{Massa específica da partícula de catalisador \nomunit{kg/m^3}}

A porosidade de cada um dos leitos pode ser calculada, portanto, pela \autoref{eq:porosidade}, como segue:
...
sendo $V_l$ o volume do leito e $\epsilon_B$ a porosidade do leito.

Utilizando os dados apresentados na \autoref{sec:catalisador}, e utilizando os valores de massa específica \emph{bulk} dos leitos (\rho_{B}), publicados por \citelonline{Rojas2014a}, chega-se nos valores mostrados na \autoref{tab:dadosdosleitos}. 
...
As condições de operação consideradas inicialmente estão na \autoref{tab:condicoesoperacao}. Na \autoref{sec:resultados} há uma discussão sobre a validade de algumas dessas variáveis aqui apresentadas.
...
\\
\hline
{Pressão de entrada ($P_{z}$)} & $5,03$ MPa \\
{Temperatura de entrada ($T_{z}$)} & $366,15$ K \\
{Vazão mássica de entrada ($F_{w,z}$)} & $2,477.10^5}$ kg/h \\
...
\nomenclature{$\rho$}{Massa específica \nomunit{kg/m^3}}
...
Para determinar $k_{LS}$, foi utilizada a equação de \cite{Hirose1976} para o
...
Visto que a pressão de operação do reator é muito acima da pressão atmosféricas, a equação usada para estimar a retenção de líquido \epsilon_L foi a proposta por \citeonline{Larachi1991}, que, \citeonline{Ranade2011}, é uma dentre outras equações levantadas para sistemas de alta pressão \cite{Ancheyta2011}.

\begin{equation}
log \left (1-\dfrac{\epsilon_{z}^L}{\epsilon_B}) = -\dfrac{1,22We_{L,z}^{0,15}}{Re_{L,z}^{0,20}X_{G,z}^{0,15}}
\label{eq:epsilonL}
\end{equation}

onde $We$ é o número de Webber e $X$ é o número de Lockhart-Martinelli \cite{Lockhart1949}. Ambos são definidos como segue:

\begin{equation}
We_{L,z} = \dfrac{u_{L,z}^2d_p\rho_{L,z}}{\sigma_{L,z}}
\label{eq:webber}
\end{equation}
